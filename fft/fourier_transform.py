...
def lowpass_filter(t, sigRe, sigIm, options, fRef = [], spcRef = [], parameters = {}):
    # Set the Nyquist frequency
    dt = t[1] - t[0]
    fNuq = 1 / (2*dt)
    #print 'Nyquist frequency', fNuq 
    
    # Set the filter
    if (options['filter'] == 'Butterworth'):
        # Calculate the Numerator (b) and denominator (a) polynomials of the IIR filter
        b, a = signal.butter(options['order'], options['cutoff']/fNuq, btype='low', analog=False)    
    elif (options['filter'] == 'Chebyshev'):
        # Calculate the Numerator (b) and denominator (a) polynomials of the IIR filter
        b, a = signal.cheby2(options['order'], options['minAtt'], options['cutoff']/fNuq, btype='low', analog=False) 

    # Filter the time trace
    sigReFiltered = np.real(signal.filtfilt(b, a, sigRe))
    plot_timetrace([t, t], [sigRe, sigReFiltered], ['Re(signal)', 'initial'])
    
    # Compute the frequency response of a digital filter
    w, h = signal.freqz(b, a, worN=t.size)
    w = w * fNuq / np.pi
    w = np.append(-w[::-2], w)
    h = np.append(h[::-2], h)
    h = np.abs(h)
        
    # Do FFT on the filtered time trace
    [f, spc, fRaw, spcRaw] = FFT(t, sigReFiltered, sigIm, parameters)
    [fig, axes] = plot_spectrum([fRef, w, f], [spcRef/np.amax(spcRef), h, spc/np.amax(spc)], ['Re(spc)', 'lowpass filter', 'Re(filtered spc)'])
    axes.axvline(options['cutoff'], color='r', linestyle='--', linewidth = 1)
    if (options['filter'] == 'Butterworth'):
        axes.plot(options['cutoff'], 0.5*np.sqrt(2), 'ko')
    elif (options['filter'] == 'Chebyshev'):
        axes.plot(options['cutoff'], pow(10, -options['minAtt']/20.0), 'ko')
...
if __name__ == '__main__':
    ...
    parameters['bckg_start'] = -100 
    parameters['appod_func'] = 'Hamming'
    parameters['zerofil_length'] = 1 
    ...
    # # Filter the time trace
    # # filterOptions | filter    | Chebyshev / Butterworth
    # #                 order     | Polynomial order
    # #                 minAtt    | Attenuation level for Chebyshev
    # #                 cutoff    | Any frequency below the Nyquist frequency
    # filterOptions = {}
    # filterOptions['filter'] = 'Chebyshev'
    # filterOptions['order'] = 8
    # filterOptions['minAtt'] = 40
    # filterOptions['cutoff'] = 12
    # lowpass_filter(t, sigRe, sigIm, filterOptions, f, spc, parameters)
