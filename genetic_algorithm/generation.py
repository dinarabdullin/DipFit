...
class Generation:
	...
	def tournament_selection(self):
		...
		if self.chromosomes[idx1].fitness < self.chromosomes[idx2].fitness:
			return idx1
		else:
			return idx2
	...
	def score_chromosomes(self, indices, fixed, sim, exp, spinA, spinB, calc):
		func = partial(fitness_function, indices=indices, fixed=fixed, sim=sim, exp=exp, spinA=spinA, spinB=spinB, calc=calc)
		var = []
		for i in range(self.size): 
			var.append(self.chromosomes[i].genes)
		pool = Pool()
		fitness = pool.map(func, var)
		pool.close()
		pool.join()
		for i in range(self.size):
			self.chromosomes[i].fitness = fitness[i]	
